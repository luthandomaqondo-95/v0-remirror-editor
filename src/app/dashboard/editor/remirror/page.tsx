...
export default function AFS(props: { params: Params }) {
	...
	return (
		<div className="h-screen flex flex-col bg-background">
			{/* Header */}
			<header className="relative flex h-12 items-center gap-2 sm:gap-3 px-2 sm:px-4 border-b bg-background shrink-0 min-w-0">
				<Button variant="ghost" size="sm" className="gap-1.5 shrink-0 h-8 px-2" onClick={() => router.back()}>
					<ChevronLeft className="h-4 w-4" />
					<span className="hidden sm:inline">Back</span>
				</Button>
				<div className="hidden md:flex items-center gap-2 min-w-0 flex-1 max-w-[200px] lg:max-w-[280px]">
					<FileText className="h-4 w-4 shrink-0 text-muted-foreground" />
					<input
						type="text"
						value={projectName}
						onChange={(e) => setProjectName(e.target.value)}
						className="text-sm lg:text-base font-medium bg-transparent border-none outline-none focus:outline-none px-1.5 py-0.5 rounded hover:bg-muted/50 focus:bg-muted/50 min-w-0 w-full"
						placeholder="Untitled Project"
					/>
				</div>
				<div className="flex items-center gap-1.5 sm:gap-2 ml-1 sm:ml-2 min-w-0 shrink">
					<Button
						variant="ghost"
						size="sm"
						className="gap-1.5 h-8 px-2 shrink-0 cursor-pointer"
					...
					{isSaving ? (
						...
						<div className="flex items-center gap-1.5 text-amber-500 text-xs">
							...
							<span className="hidden sm:inline truncate">Unsaved</span>
						</div>
					) : lastSaved ? (
						<div className="hidden sm:flex items-center gap-1.5 text-green-600 text-xs">
				...
				<StepIndicator
					...
					className="absolute right-0 sm:right-28 mx-0 top-1 mb-0 max-w-[200px] sm:max-w-[220px] md:max-w-[260px] lg:max-w-[320px]"
				/>
